...
| **The interview** | Seven base questions, up to twelve for complex skills, asked one at a time, each one deciding a layer or a behaviour. Skips anything the user already told you. |
...
Work through these in order. Record each in `STATUS.md`'s decisions table before moving to the next. The orientation is an offer, not a gate — if the user waves it off, skip it and carry the weight yourself by making each recommendation concrete rather than abstract.

**If the user hands the rest back to you** — "just pick," "you know my setup," any version of tapping out — present the outstanding decision points **together, each with its stated default and one line of why**, and confirm them as a single group. That is not picking silently: every default is named, and rejecting any one of them costs the user a single word. Six sequential decision points is past most people's patience, and a user who has stopped reading is worse than a user who approved a batch. What is never allowed is collapsing them to "I'll handle it" with nothing shown.
...
- **C. Canonical in the workspace, routed by an instructions line** — for platforms with no skills directory: the folders live in the workspace and the standing instructions file says "when the user asks about X, read `<path>/SKILL.md` and follow it." *Trade-off:* the routing line is manual and grows with every skill; past ~10 skills it needs an index line instead of one line per skill.
  *Path style for C:* write the routing line with a **workspace-relative** path (`skills/<name>/SKILL.md`) whenever the instructions file lives inside the workspace — that way it still resolves after a clone, on a teammate's machine, or when the folder moves. Use an absolute path only when the instructions file sits outside the workspace (a global config), where relative means nothing. Record which you used.
...
**A written house convention outranks this recommendation.** Phase 0 step 4 asks you to note the naming style already in use — this is the decision that finding is for. If the workspace states a rule (a conventions note, a repo or vault style guide, or a folder tree that obviously and consistently follows one), follow it and say which one you're following and why. Don't hand a terse user an aesthetic argument against their own documented convention; the convention is already their answer to this question.
...
**Recommendation:** **C if the user already keeps project folders with status files** — the habit exists, the folder gets read, and voice-tuned skills are exactly the ones whose decisions get forgotten. **B if they don't** — start where the payoff is unambiguous, and add Tier 2 later if they find themselves wanting it. **A only if the user is solo, building a handful of simple skills, and says outright they won't read the folder** — an unread status file is worse than none, because it looks authoritative while going stale. Default: **C if the scan found at least one project folder containing a state file** — a `STATUS.md`, `PROGRESS.md`, a running log, anything that gets *updated* rather than written once. A `README.md` on its own doesn't count: it proves they document, not that they maintain. **Otherwise B.** If the two signals disagree — a team that keeps no state files, or a solo operator who keeps meticulous ones — **the state-file habit wins**, because it predicts whether the folder gets read; team size only predicts whether it *should* be. Record the parent path. The default is a `projects/<skill-name>/` folder in the workspace — **but only if `projects/` isn't already carrying a different meaning.** In plenty of workspaces it holds client engagements, delivery work or active jobs, and dropping skill-build folders in among them corrupts a tree the user relies on. Look at what's actually in it first; if it's taken, put the companion folders under the skills folder instead (`<skills>/builds/<skill-name>/`) and record that path.
...
**Options:**
- **A. A fresh subagent** — you launch one, hand it the cold-read payload specified in `skill-creation/SKILL.md` Step 8a (the new `SKILL.md`, one approved prompt, and the *paths* of every reference and workspace file the skill reads — nothing from the build conversation), and read its output. *Trade-off:* needs a platform that runs subagents. Note the scope: a subagent can *dry-run a finished skill*, which is all this gate asks. It cannot run the build funnel itself, because the spec block requires the user's confirmation and a subagent has no user — never delegate a whole skill build to one.
- **B. A fresh session the user drives** — the user opens a new chat or composer, pastes a prepared payload, and brings the result back. *Trade-off:* a real stop; you wait for them. It is also the *better* test — a genuinely separate context, not a child of yours. **What makes it cold is the empty context, not a second person:** a solo operator opening a new composer gets a real cold read, as long as only the payload goes into it. Pasting the build conversation alongside turns B into C without anyone noticing.
  **Don't ask the user to assemble the payload — write it for them.** Save a file (`_dryrun-<skill-name>.md`, kept *outside* any directory the platform scans for skills) containing the new `SKILL.md` and one approved prompt at the bottom. **List the reference files by path rather than pasting them in**, whenever the cold session can open files — that is the only thing that makes checklist item 2 observable, because a pasted reference has been read before the step that says to read it. Inline them only when the cold session is a plain chat window that can't open files, and then say at the top of the payload: "checklist item 2 is not testable in this configuration — report it untested, not passed." Hand over the path with the matching instruction: if the cold session can open files, "Open a new session in this workspace, point it at this file, and send me back what it produces"; if it can't, "Open a new chat, paste this whole file, and send me back what it produces." Either way: "Don't add anything else." Delete the file once the gate passes.
...
2. Do the `references/` and `scripts/` get loaded at the steps that need them — and not earlier? *(Only answerable if the cold reader could open the files itself. Pasted-in contents make this untestable — report it untested, never passed.)*
...
1. **Install the skill** at the DP-1 location. The folder is named `skill-creation` — plus the DP-2 prefix on the *storage* folder if prefixes are on, never on whatever the platform reads.
   ...
   - **Resolve the placeholders in the installed copy.** `skill-creation/SKILL.md` ships with `<skills>` written as a placeholder; in the copy you install, replace every occurrence with the real path chosen above. Leaving them is how a future session ends up reading "the install record at `<skills>/README.md`" and having to guess what `<skills>` means — the file that answers the question is the one it can't find. **One paragraph is not a substitution target — delete it.** `skill-creation/SKILL.md`'s install-time note carries a self-healing paragraph that begins "`<skills>` should have been replaced with a real path…". It exists only for the case where you *skipped* this step, so substituting into it produces nonsense ("`skills` should have been replaced… if you are still reading the literal text `skills`"). Once you have resolved the real paths, delete that paragraph outright.

Grep the installed copy for `<` afterwards. What should be gone: `<skills>`, which names a real location. What legitimately stays: the illustrative placeholders in the references — `<name>`, `<triggers>`, `<skill-name>`, `<path>`, `<discovery dir>` — which are generic by design and get filled in per skill, not per install. If you're unsure which kind you're looking at, ask whether it names a place that exists right now: if yes, resolve it; if no, leave it.
...
4. **Wire the standing instructions file.** This is the stickiness step — an install that only creates folders dies at the user's first new chat.
   ...
   **Merge, or a new file?** If the platform uses a single instructions *file* (`CLAUDE.md`, `AGENTS.md`, `GEMINI.md`), append to it in the user's own format — read it first, match its headings, never rewrite what's there. If the platform uses a rules *directory* (`.cursor/rules/*.mdc`), **a new scoped file is the merge** — that is how the directory is designed to work, and editing their existing rule file risks clobbering rules you don't understand. Either way, read what already exists first: if a rule already covers skill routing, extend it instead of adding a competing one. Then add:
   ...
   > **Building skills.** Before building anything new, check `<skills>/README.md` — a skill may already cover it. When the user wants to automate a repeating workflow, turn a prompt into something reusable, or says "build me a skill", use the `skill-creation` skill — **on a routed install, write this as "read `<path>/skill-creation/SKILL.md` and follow it"**, since there is no skill mechanism to name. It gates whether the thing should be a skill at all, then scaffolds, installs and tests it. Every new skill gets registered in that index.
...
2. **Run `skill-creation/SKILL.md` end to end, out loud:** the Step 0 gate (say the verdict, don't just think it) → tier set silently → interview per DP-6 → **spec block, confirmed** → write the folder → install and verify discovery → generate realistic test prompts → **dry-run per DP-4, all four checklist items reported, including the value check** (on DP-4 = B, write the paste payload to a file and hand over the path — don't make the user assemble it) → register in `<skills>/README.md`.
   **Retire what the new skill replaced.** If the candidate came from a saved prompt, don't leave the old version sitting there — the next person to reach for it gets stale instructions, and the skill quietly stops being the source of truth. **Replace its contents with a single pointer line** ("Superseded by the `<skill-name>` skill — see `<path>/SKILL.md`") rather than deleting it. Retired files still get opened: a pointer redirects whoever goes looking, where a deleted file just makes them rewrite the prompt from memory. Delete outright only if the user asks. Never leave it emptied-but-present, which is worse than either.
